"use strict";
// добавить:
	// https://itchief.ru/lessons/php/feedback-form-for-website#comment-5463
	// https://itchief.ru/lessons/php/feedback-form-for-website#comment-5565
...
$(function () {
	...
	var ProcessForm=function (parameters) {
		...
		this.existenceUploadsFile=parameters['existenceUploadsFile'] || true;// поправить
	...
	*/
	...
	ProcessForm.prototype.collectData=function () {
		this.changeStateImages(true); // отключаем отправку файлов (disabled), не удовлетворяющиx требованиям
					...
					for (var error in data) {
						if (data.hasOwnProperty(error)) {
							...
							if (error !== 'info' && error !== 'log') { // кроме тех, которые имеют ключ info или log
								$.jGrowl(data[error], {theme: 'jgrowl-error', life: 15000});
							...
							if (error === 'info') { // выводим все сообщения с ключом info с помощью jGrowl
								data[error].forEach(function (info, i, error) {
									$.jGrowl(info, {theme: 'jgrowl-error', life: 15000});
								});
							}
